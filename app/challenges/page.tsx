  ...
  return (
    <main className="min-h-dvh bg-pp-bg-dark">
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute -top-40 -right-40 w-96 h-96 rounded-full bg-red-400/8 blur-3xl" />
        <div className="absolute bottom-0 -left-40 w-80 h-80 rounded-full bg-pp-accent/8 blur-3xl" />
      </div>
      ...
      <header className="sticky top-0 z-30 bg-pp-bg-dark/90 backdrop-blur-sm border-b border-white/6">
        <div className="max-w-lg mx-auto px-5 py-3.5 flex items-center gap-3">
          ...
          <span className="font-display text-base text-white font-semibold">Challenges</span>
        </div>
      </header>

      <div className="relative z-10 max-w-lg mx-auto px-5 py-6">
        <ChallengesPanel coupleId={state.coupleId} currentProfile={state.profile} />
      </div>
    </main>
  );
}
